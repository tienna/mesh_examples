  ...
  async submitTx(tx) {
    try {
      ...
      const { data, status } = await this._axiosInstance.post(
        "tx/submit",
        this.submitTxToBytes ? (0, import_common2.toBytes)(tx) : tx,
        { headers }
      );
      if (status === 200 || status == 202) return data;
      throw parseHttpError(data);
    ...
    if (scriptHash) {
      ...
      if (status === 200 || status == 202) {
        ...
        if (data.type.startsWith("plutus")) {
          ...
          const normalized = (0, import_core_cst.normalizePlutusScript)(plutusScript, "DoubleCBOR");
          script = {
...
// src/hydra/hydra-connection.ts
var HydraConnection = class extends import_events.EventEmitter {
  _websocket;
...
var HydraProvider = class {
  ...
  constructor({ url, history = false }) {
    ...
    this._axiosInstance = import_axios4.default.create({ baseURL: url });
  }
  async connect() {
  ...
  async fetchProtocolParameters(epoch = Number.NaN) {
    try {
      ...
      if (status === 200) {
        const protocolParams = (0, import_common3.castProtocol)({
...
var KoiosProvider = class {
  ...
  constructor(...args) {
    if (typeof args[0] === "string" && args[0].startsWith("http")) {
      ...
      this._axiosInstance = import_axios5.default.create(config);
    }
  }
  async fetchAccountInfo(address) {
    try {
  ...
  async fetchAssetMetadata(asset) {
    try {
      ...
      if (status === 200)
        return {
          ...data[0].minting_tx_metadata[721][policyId][(0, import_common4.toUTF8)(assetName)],
  ...
  async fetchHandleAddress(handle) {
    ...
    try {
      const assetName = (0, import_common4.fromUTF8)(handle.replace("$", ""));
      const { data, status } = await this._axiosInstance.get(
        `asset_addresses?_asset_policy=${import_common4.SUPPORTED_HANDLES[1]}&_asset_name=${assetName}`
      );
  ...
  resolveScriptRef = (kScriptRef) => {
    if (kScriptRef) {
      ...
      if (kScriptRef.type.startsWith("plutus")) {
        const normalized = (0, import_core_cst2.normalizePlutusScript)(kScriptRef.bytes, "DoubleCBOR");
        script = {
          code: normalized,
          version: kScriptRef.type.replace("plutus", "")
        };
      } else {
        script = (0, import_core_cst2.fromNativeScript)((0, import_core_cst2.deserializeNativeScript)(kScriptRef.bytes));
      }
...
var MaestroProvider = class {
  ...
  async fetchHandle(handle) {
    ...
    try {
      ...
      const asset = await this.fetchAssetMetadata(
        `${import_common5.SUPPORTED_HANDLES[1]}000643b0${assetName}`
      );
  ...
  resolveScript = (utxo) => {
    if (utxo.reference_script) {
      ...
      if (utxo.reference_script.type === "native") {
        ...
        if (scriptBytes) {
          const normalized = (0, import_core_cst3.normalizePlutusScript)(scriptBytes, "DoubleCBOR");
          script = {
            code: normalized,
...
var OgmiosProvider = class {
  _baseUrl;
  constructor(...args) {
    this._baseUrl = (0, import_common6.isNetwork)(args[0]) ? import_common6.SUPPORTED_OGMIOS_LINKS[args[0]] : args[0];
  }
  async evaluateTx(tx) {
...
var U5CProvider = class {
  ...
  onTxConfirmed(txHash, callback, limit = 100) {
    const onConfirmed = async () => {
      const updates = this.submitClient.waitForTx((0, import_common7.hexToBytes)(txHash));
   ...
   */
  async submitTx(tx) {
    const cbor = (0, import_common7.toBytes)(tx);
    const hash = await this.submitClient.submitTx(cbor);
    return (0, import_common7.bytesToHex)(hash);
  }
  /**
   * Obtain information about a specific stake account.
   * @param address - Wallet address to fetch account information
   ...
   */
  async fetchAddressUTxOs(address, asset) {
    const addressBytes = Buffer.from(import_core_cst4.Address.fromBech32(address).toBytes());
  ...
  awaitTransactionConfirmation(txId, timeout) {
    const onConfirmed = (async () => {
      const updates = this.submitClient.waitForTx((0, import_common7.hexToBytes)(txId));
      for await (const stage of updates) {
        if (stage == import_spec.submit.Stage.CONFIRMED) {
          return true;
        }
  ...
  _rpcUtxoToMeshUtxo(rpcTxoRef, rpcTxOutput) {
    ...
    return {
      ...
      output: {
        address: import_core_cst4.Address.fromBytes(
          import_core_cst4.CardanoSDKUtil.HexBlob.fromBytes(rpcTxOutput.address)
        ).toBech32(),
        amount,
        dataHash,
        plutusData,
  ...
  _rpcPParamsToProtocol(rpcPParams) {
    return (0, import_common7.castProtocol)({
...
var YaciProvider = class {
  ...
  constructor(baseUrl = "https://yaci-node.meshjs.dev/api/v1/", adminUrl) {
    ...
    if (adminUrl) {
      this._adminAxiosInstance = import_axios7.default.create({
        baseURL: adminUrl
      });
    }
  }
  ...
  resolveScriptRef = async (scriptHash) => {
    if (scriptHash) {
      ...
      if (status === 200) {
        ...
        if (data.type.startsWith("plutus")) {
          const plutusScript = await this.fetchPlutusScriptCBOR(scriptHash);
          const normalized = (0, import_core_cst5.normalizePlutusScript)(plutusScript, "DoubleCBOR");
          script = {
            version: data.type.replace("plutus", ""),
            code: normalized
          };
        ...
        return (0, import_core_cst5.toScriptRef)(script).toCbor();
      }
      throw parseHttpError(data);
    }
    return void 0;
  ...
  async fetchHandleAddress(handle) {
    try {
      const assetName = (0, import_common8.fromUTF8)(handle.replace("$", ""));
      const { data, status } = await this._axiosInstance.get(
        `assets/${import_common8.SUPPORTED_HANDLES[1]}${assetName}/addresses`
      );
...
var OfflineFetcher = class _OfflineFetcher {
  ...
  async fetchHandle(handle) {
    try {
      ...
      const handleAsset = `${import_common9.SUPPORTED_HANDLES[1]}000de140${assetName}`;
  ...
  async fetchHandleAddress(handle) {
    const assetName = (0, import_common9.fromUTF8)(handle.replace("$", ""));
    const policyId = import_common9.SUPPORTED_HANDLES[1];
