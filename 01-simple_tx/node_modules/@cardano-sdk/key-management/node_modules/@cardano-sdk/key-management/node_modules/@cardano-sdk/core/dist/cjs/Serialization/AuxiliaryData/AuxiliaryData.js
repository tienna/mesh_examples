...
exports.AuxiliaryData = exports.ALONZO_AUX_TAG = exports.SHELLEY_ERA_FIELDS_COUNT = void 0;
const CBOR_1 = require("../CBOR");
const GeneralTransactionMetadata_1 = require("./TransactionMetadata/GeneralTransactionMetadata");
const util_1 = require("@cardano-sdk/util");
const Scripts_1 = require("../Scripts");
const Script_1 = require("../../Cardano/types/Script");
const errors_1 = require("../../errors");
const misc_1 = require("../../util/misc");
exports.SHELLEY_ERA_FIELDS_COUNT = 2;
exports.ALONZO_AUX_TAG = 259;
class AuxiliaryData {
    constructor() {
    ...
    toCbor() {
        ...
        if (elementsSize === 1 && __classPrivateFieldGet(this, _AuxiliaryData_metadata, "f") && __classPrivateFieldGet(this, _AuxiliaryData_metadata, "f").metadata().size > 0) {
            writer.writeEncodedValue((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _AuxiliaryData_metadata, "f").toCbor()));
        }
        else if (elementsSize === exports.SHELLEY_ERA_FIELDS_COUNT &&
            __classPrivateFieldGet(this, _AuxiliaryData_metadata, "f") &&
            __classPrivateFieldGet(this, _AuxiliaryData_metadata, "f").metadata().size > 0 &&
            __classPrivateFieldGet(this, _AuxiliaryData_nativeScripts, "f") &&
            ...
            for (const script of __classPrivateFieldGet(this, _AuxiliaryData_nativeScripts, "f")) {
                writer.writeEncodedValue((0, misc_1.hexToBytes)(script.toCbor()));
            }
        }
        else {
            writer.writeTag(exports.ALONZO_AUX_TAG);
    ...
    static fromCbor(cbor) {
        ...
        if (peekState === CBOR_1.CborReaderState.StartMap) {
            __classPrivateFieldSet(auxData, _AuxiliaryData_metadata, GeneralTransactionMetadata_1.GeneralTransactionMetadata.fromCbor(util_1.HexBlob.fromBytes(reader.readEncodedValue())), "f");
        }
        else if (peekState === CBOR_1.CborReaderState.Tag) {
            const tag = reader.readTag();
            if (tag !== exports.ALONZO_AUX_TAG) {
                throw new util_1.InvalidArgumentError('cbor', `Expected tag '${exports.ALONZO_AUX_TAG}', but got ${tag}.`);
            }
            reader.readStartMap();
                ...
                switch (key) {
                    ...
                    case 1n:
                        ...
                        while (reader.peekState() !== CBOR_1.CborReaderState.EndArray) {
                            auxData.nativeScripts().push(Scripts_1.NativeScript.fromCbor(util_1.HexBlob.fromBytes(reader.readEncodedValue())));
                        }
                        reader.readEndArray();
                        break;
                    case 2n: {
                        ...
                        reader.readStartArray();
                        while (reader.peekState() !== CBOR_1.CborReaderState.EndArray) {
                            auxData.plutusV1Scripts().push(Scripts_1.PlutusV1Script.fromCbor(util_1.HexBlob.fromBytes(reader.readEncodedValue())));
                        }
                        reader.readEndArray();
                        break;
                    }
                    case 3n: {
                        ...
                        reader.readStartArray();
                        while (reader.peekState() !== CBOR_1.CborReaderState.EndArray) {
                            auxData.plutusV2Scripts().push(Scripts_1.PlutusV2Script.fromCbor(util_1.HexBlob.fromBytes(reader.readEncodedValue())));
                        }
                        reader.readEndArray();
                        break;
                    }
                    case 4n: {
                        ...
                        reader.readStartArray();
                        while (reader.peekState() !== CBOR_1.CborReaderState.EndArray) {
                            auxData.plutusV3Scripts().push(Scripts_1.PlutusV3Script.fromCbor(util_1.HexBlob.fromBytes(reader.readEncodedValue())));
                        }
                        reader.readEndArray();
                        break;
                    }
    ...
    static fromCore(auxData) {
        ...
        if (auxData.blob) {
            result.setMetadata(GeneralTransactionMetadata_1.GeneralTransactionMetadata.fromCore(auxData.blob));
        }
        if (auxData.scripts) {
...
_a = AuxiliaryData, _AuxiliaryData_metadata = new WeakMap(), _AuxiliaryData_nativeScripts = new WeakMap(), _AuxiliaryData_plutusV1Scripts = new WeakMap(), _AuxiliaryData_plutusV2Scripts = new WeakMap(), _AuxiliaryData_plutusV3Scripts = new WeakMap(), _AuxiliaryData_originalBytes = new WeakMap(), _AuxiliaryData_instances = new WeakSet(), _AuxiliaryData_getMapSize = function _AuxiliaryData_getMapSize() {
    ...
    for (const script of scripts) {
        switch (script.__type) {
            case Script_1.ScriptType.Native:
                if (!result.native)
                    result.native = new Array();
                result.native.push(Scripts_1.NativeScript.fromCore(script));
                break;
            case Script_1.ScriptType.Plutus:
                if (script.version === Script_1.PlutusLanguageVersion.V1) {
                    if (!result.plutusV1)
                        result.plutusV1 = new Array();
                    result.plutusV1.push(Scripts_1.PlutusV1Script.fromCore(script));
                }
                else if (script.version === Script_1.PlutusLanguageVersion.V2) {
                    if (!result.plutusV2)
                        result.plutusV2 = new Array();
                    result.plutusV2.push(Scripts_1.PlutusV2Script.fromCore(script));
                }
                else if (script.version === Script_1.PlutusLanguageVersion.V3) {
                    if (!result.plutusV3)
                        result.plutusV3 = new Array();
                    result.plutusV3.push(Scripts_1.PlutusV3Script.fromCore(script));
                }
                break;
            default:
