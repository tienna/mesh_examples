...
export const somePartialUtxos = [
    [
        {
            address: Cardano.PaymentAddress('addr_test1qr4m502gr9hnaxac5mxjln22jwavf7pcjmh9sw7fujdvgvj9ef6afquphwg7tj4mmm548m3t50hxfyygjuu222kx96eshcathg'),
            index: 0,
    ...
    [
        {
            address: Cardano.PaymentAddress('addr_test1qz2fxv2umyhttkxyxp8x0dlpdt3k6cwng5pxj3jhsydzer3jcu5d8ps7zex2k2xt3uqxgjqnnj83ws8lhrn648jjxtwq2ytjqp'),
            index: 0,
...
export const createStubUtxoProvider = (utxos = somePartialUtxos, delayMs) => ({
    healthCheck: async () => {
        if (delayMs)
            await delay(delayMs);
        return { ok: true };
    },
    utxoByAddresses: async ({ addresses }) => {
        if (delayMs)
            await delay(delayMs);
        return utxos.filter((u) => addresses.includes(u[0].address) && addresses.includes(u[1].address));
    }
});
//# sourceMappingURL=createStubUtxoProvider.js.map